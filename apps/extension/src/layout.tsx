...
const fullPanelPaths = ['/settings', '/signtransaction', '/login'];
...
export const GlobalLayout = (props: GlobalLayoutProps) => {
  ...
  return (
    <>
      {!isFullScreen && (
        <div id='drawer-boundry' className={cn('relative overflow-auto', !isFullScreen ? 'panel' : '')}>
          {props.children}
      ...
      {isFullScreen && (
        <div id='drawer-boundry' className={cn('relative m-auto overflow-auto', !isFullScreen ? 'panel' : '')}>
          {props.children}
        </div>
      )}
    </>
